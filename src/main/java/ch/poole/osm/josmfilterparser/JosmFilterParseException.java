...
public class JosmFilterParseException extends ParseException {
    ...
    JosmFilterParseException(String message) {
        this(message, -1, -1);
    }

    JosmFilterParseException(String message, @Nullable Token token) {
        this(message, token != null ? token.beginLine : -1, token != null ? token.beginColumn : -1);
    }

    private JosmFilterParseException(String message, int line, int column) {
        super(message);
        this.line = line;
        this.column = column;
    }

    JosmFilterParseException(ParseException ex) {
